...
Lithium_SCMD <- read_excel("lithium-full.xlsx") # Secondary care Lithium dataset for 2019-2024

#### Cleaning/ Organising dataset for plotting ########
# Mapping regions
org_mapping <- read_excel("org-mapping.xlsx") # importing dataset that can map ods_code to regions in secondary care dataset
...
ggplot(Secondary_DDD_by_year, aes(x = as.factor(year), y = total_DDD / 1e6)) +
...
geojson_file <- "new_nuts_shp.txt" #where you have saved your geojson_file
geojson_data <- st_read(geojson_file)

# Save the object as an RDS file
saveRDS(geojson_data, file = "new_nuts_shp.rds")

# Load the RDS file
nuts_shp <- readRDS("new_nuts_shp.rds")
#View(nuts_shp)
...
# getting the sum of DDD per region (2024 total DDD for eacg region/ regional population estimates)
...
# Filtering 2024 and sum total DDD by region
...
#View(coverage_data)
#print(coverage_data[, c("region", "DDD/population")])

# Extract unique values for breaks and labels
unique_values <- sort(unique(coverage_data$`DDD/population`))
breaks <- c(0, unique_values, max(unique_values, na.rm = TRUE) + 10)
labels <- scales::label_number(breaks)

# Plot with specified color scale
legend_labels <- c( "0" = "0",paste0(sort(round(secondary_lithium_df$`DDD/population`, 1)), " DDD/population"), "120" = "120")

secondary_coverage_plot <- coverage_data %>%
  ggplot(aes(geometry = geometry, fill = `DDD/population`)) +
...
PrimaryCare_Lithium <- read.csv(gzfile("primary_lithium.csv.gz")) # Primary care Lithium dataset for 2015-2024
...
Practice_codes <- read_excel('practice_codes.xlsx')  
...
# Accompanying bar plot for prescribing trends over time
ggplot(Primaryy_DDD_by_year, aes(x = as.factor(year), y = total_DDD / 1e6)) +
...
# getting the sum of DDD per region  - for 2024 DDD / population
...
# Join the datasets ~
coverage_data <- nuts_shp %>%
  left_join(primary_lithium_df, by = "Region")  # Join using the correct column names
...
# Plot with specified color scale
legend_labels <- c( "0" = "0",paste0(sort(round(primary_lithium_df$`DDD/population`, 1)), " DDD/population"), "120" = "120")

primary_coverage_plot <- coverage_data %>%
  ggplot(aes(geometry = geometry, fill = `DDD/population`)) +
  geom_sf(lwd = 0.8, colour = 'black') +
  geom_sf_label(aes(label = Region),
                colour = "white",
...
#### FP10 analysis ####
# 2017-2024
Hospital_FP10_data <- read_excel("FP10_data (1).xlsx")
...
Hospital_FP10_data2024 <- read_excel("FP10_2024.xlsx")
...
# REGION MAPPING
org_mapping <- read_excel("org-mapping.xlsx")
...
# Join the datasets ~
coverage_data <- nuts_shp %>%
  left_join(Hospital_FP10_total_DDD_by_region_2024, by = "region")  # Join using the correct column names

#View(coverage_data)
#print(coverage_data$`DDD/population`)
#print(coverage_data$region)
...
# Plot with specified color scale
legend_labels <- c( "0" = "0",paste0(sort(round(Hospital_FP10_total_DDD_by_region_2024$`DDD/population`, 1)), " DDD/population"), "120" = "120")

FP10_coverage_plot <- coverage_data %>%
  ggplot(aes(geometry = geometry, fill = `DDD/population`)) +
  geom_sf(lwd = 0.8, colour = 'black') +
  geom_sf_label(aes(label = region),
                colour = "white",
...
ggplot(combined_df_all, aes(x = Region, y = `DDD/population`, fill = Source)) +
...
# Combine and clean the data as before
ggplot(summed_by_region, aes(x = year, y = total_DDD_pop_sum , color = region)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  labs(
